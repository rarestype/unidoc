...
extension SSGC {
    @frozen public struct Toolchain {
        private let appleSDK: AppleSDK?
        public let splash: Splash
        private let paths: Paths

        private let recoverFromAppleBugs: Bool
...
extension SSGC.Toolchain {
    func manifest(
        ...
        let json: JSON
        do {
            let utf8: [UInt8] = try file.open(
                .readWrite,
                permissions: (.rw, .r, .r),
                options: [.create, .truncate]
            ) {
                let dump: SystemProcess = try .init(
...
extension SSGC.Toolchain {
    ...
    private func dump(
        ...
        do {
            ...
            guard self.recoverFromAppleBugs else {
