...
extension SSGC {
    @frozen public struct Workspace: Equatable {
        public let location: FilePath.Directory

        private init(absolute location: FilePath.Directory) {
            self.location = location
        }
    }
}
extension SSGC.Workspace {
    @inlinable public var cache: FilePath.Directory { self.location / "cache" }
...
extension SSGC.Workspace {
    ...
    public func buildStandardLibrary(
        with swift: SSGC.Toolchain,
        validation: SSGC.ValidationBehavior = .ignoreErrors,
        clean: Bool = true
    ) throws -> SymbolGraphObject<Void> {
