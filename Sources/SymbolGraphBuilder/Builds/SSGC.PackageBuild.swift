...
extension SSGC.PackageBuild {
    private func listExtraManifests() throws -> [MinorVersion] {
        ...
        for file: Result<FilePath.Component, FileError> in self.root {
            ...
            case "Package@swift" = name[..<hyphen] else {
                continue
            }
...
extension SSGC.PackageBuild {
    ...
    public static func local(
        project location: FilePath.Directory,
        ...
        let scratch: SSGC.PackageBuildDirectory = .init(
            configuration: .debug,
            location: location / scratchName
        )

        return .init(
            id: .unversioned(.init(last.string)),
            scratch: scratch,
            cache: nil,
            flags: flags,
            root: location,
    ...
    public static func remote(
        ...
        let pin: SPM.DependencyPin = .init(
        ...
        return .init(
            id: .versioned(pin, ref: refName, date: checkout.date),
            scratch: scratch,
...
extension SSGC.PackageBuild {
    @_spi(testable) public func compileBook(
        updating status: SSGC.StatusStream? = nil,
        with toolchain: SSGC.Toolchain
    ) throws -> (SymbolGraphMetadata, SSGC.BookSources) {
        switch self.id {
        ...
        let sources: SSGC.BookSources = .init(
            modules: modules,
    ...
    @_spi(testable) public func compileSwiftPM(
        updating status: SSGC.StatusStream? = nil,
        with toolchain: SSGC.Toolchain,
        clean: Bool = true
    ) throws -> (SymbolGraphMetadata, SSGC.PackageSources) {
        if  clean {
        ...
        let modules: SSGC.ModuleGraph = try .package(
            sink: packageNodes.sink,
        ...
        let symbolsCached: FilePath.Directory = try toolchain.dump(
            stdlib: stdlib,
