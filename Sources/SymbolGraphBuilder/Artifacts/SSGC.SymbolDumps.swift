...
extension SSGC {
    @_spi(testable) public struct SymbolDumps {
        let modules: [Symbol.Module: SymbolFiles]

        private init(modules: [Symbol.Module: SymbolFiles]) {
            self.modules = modules
        }
    }
}
extension SSGC.SymbolDumps {
    @_spi(testable) public static func collect(
        from locations: FilePath.Directory...
    ) throws -> Self {
    ...
    static func collect(from locations: [FilePath.Directory]) throws -> Self {
        let symbols: [Symbol.Module: SSGC.SymbolFiles] = try locations.reduce(
            ...
            for filename: Result<FilePath.Component, FileError> in $1 {
                ...
                guard
                let id: SymbolGraphPart.ID = .init("\(filename)") else {
                    continue
                }
